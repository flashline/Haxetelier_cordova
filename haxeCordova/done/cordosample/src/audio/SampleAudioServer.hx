...
class SampleAudioServer	  {
	...
	function onClickRecord (e:MouseEvent) {		
		...
		//
		name =  "recordedSound-"+(++soundOrder)+".amr";	
		path = File.externalDataDirectory ;
		media = new Media(path + name, onMediaSuccess, onMediaError ); 
		media.startRecord();
	}	
	...
	function uploadFile(pathname:String, name:String) {
		...
		//not ok : // var uri = StringTools.urlEncode("http://www.pixaline.net/intra/cordosample/sound/upload.php");
		var uri =  SampleAudioServer.SERVER_URL + "upload.php";		
	...
	function playOneFile(url:String,?response:String="No server response") {		
		...
		ply.style.visibility = "visible";	
		ply.src = url; span.innerHTML = response;
		ply.addEventListener("pause",onStopPlaying);
		ply.addEventListener("ended",onStopPlaying);
		ply.play();
	}
