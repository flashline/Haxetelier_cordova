...
class SampleAudioLocal	 {
	...
	function onClickRecord (e:MouseEvent) {		
		Capture.captureAudio(onAudioSuccess, onAudioError, { limit: 1 } );
	}
	function onAudioSuccess(mf:Array<MediaFile>) {
		...
		mediaFiles = mf;	
		//
		//trace("Fin de lecture de "+mediaFiles.length+"fichier(s)");
	}
	...
	function onClickPlay (e:MouseEvent) {	
		...
		//	
		if (mediaFiles == null || mediaFiles == []) "Auncun son enregistr\u00e9 ! ".alert();
		else {
	...
	function playOneFile(p:String, n:String) {
		...
		media = new Media(path , onMediaPlaySuccess, onMediaError ); 
		media.play();
		//trace ("Asking to play " + path); 		
	}
